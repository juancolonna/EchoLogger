...
def request(
    ...
    if auth:
        if callable(auth):
            ...
            x = encode_basic_auth(auth[0], auth[1])  
            headers["Authorization"]=x[b"Authorization"]
    ...
    while True:
        ...
        try:
            ...
            sock.connect(ai[-1])
            _https = "https"
            if not 'str' in str(type(ctx.scheme)): _https = _https.encode('utf-8')
            if ctx.scheme == _https:
                try:
                    import ssl
                except ImportError:
                    import ussl as ssl
            ...
            if not b"Host" in headers.items():
                sf.write(b"Host: %s\r\n" % ctx.host.encode())
